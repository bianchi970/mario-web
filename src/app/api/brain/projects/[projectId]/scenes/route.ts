...
async function proxyLocal(req: NextRequest, projectId: string): Promise<Response> {
  const body = ['GET', 'HEAD'].includes(req.method) ? undefined : await req.text();
  const upstream = await fetch(
    `${BRAIN_URL}/projects/${encodeURIComponent(projectId)}/scenes`,
    {
      method: req.method,
      headers: {
...
async function proxyBridge(req: NextRequest, projectId: string): Promise<Response> {
  const bodyText = ['GET', 'HEAD'].includes(req.method) ? undefined : await req.text();
  const relayPayload: Record<string, unknown> = {
    method: req.method,
    path: `/api/hub/brain/projects/${encodeURIComponent(projectId)}/scenes`,
...
async function handle(req: NextRequest, ctx: RouteContext): Promise<Response> {
  ...
  try {
    return REMOTE_BRIDGE_URL ? proxyBridge(req, projectId) : proxyLocal(req, projectId);
  } catch {
    return unavailable();
  }
}
